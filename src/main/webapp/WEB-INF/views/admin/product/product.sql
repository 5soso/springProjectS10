...
create table categoryMiddle(
  categoryMiddleCode  char(1)  not null,		 /* 중분류코드(A,B,C,...)(중복불허) */
  categoryMiddleName  varchar(20) not null,  /* 중분류명(중복불허) */
...
create table categorySub(
  categoryMiddleCode  char(1)  not null,		/* 중분류코드를 외래키로 지정 */
  categorySubCode  char(2)  not null,				/* 소분류코드(01,02,03,...) => 문자형 숫자 2자(중복불허) */
  categorySubName  varchar(20) not null, 		/* 소분류명(상품구분) */
...
create table product (
  ...
  productCode   varchar(20) not null,				/* 상품고유코드(중분류코드+소분류코드+상품고유번호) 예 : A 01 5 */
  productName   varchar(100) not null,			/* 상품명(상품모델명) */
  detail				varchar(100) not null,			/* 상품의 간단설명(초기화면 메인창에 출력할 간단한 설명) */
  mainPrice     int not null,								/* 상품의 기본가격 */
  fSName				varchar(500) not null,			/* 상품의 기본사진(1장이상 처리시는 '/'로 구분한다. */
...
create table productCart (
  ...
  thumbImg		varchar(500) not null,			/* 서버에 저장된 상품의 메인 이미지 */
...
foreign key(oIdx) references productOrder(idx) on update cascade on delete cascade --나중에 올려놓기
drop table productBaesong;



-- Test
select * from categorySub where categoryMiddleCode = 'A';
...
select product.*, midd.categoryMiddleName from product product, categoryMiddle midd 
	 		where product.categoryMiddleCode=midd.categoryMiddleCode  and midd.categoryMiddleCode = 'A' order by product.idx desc;
select w.*,p.*,s.categorySubName from wishList w, product p, categorySub s where w.productCode=p.productCode and p.categorySubCode=s.categorySubCode order by wishDate desc;