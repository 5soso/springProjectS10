  ...
  <form name="tempForm">
	  ...
	  <table class="table table-hover">
	    ...
	    <c:forEach var="vo" items="${vos}">
		    <tr>
		 		<td>${curScrStartNo}</td>
		      <td class="m-0 pt-3 pl-0 pr-3 text-right">[${vo.part}]</td>
		      <td style="text-align:left;" class="m-0 pt-3 pl-0 text-left">
		        <c:if test="${vo.qnaSw == 'a'}"> &nbsp;&nbsp; └</c:if>
		        <c:if test="${!empty vo.pwd}"><i class="fas fa-lock"></i></c:if>
		        <c:if test="${vo.pwd == ''}"><a href="adminQnaContent?pag=${pageVO.pag}&idx=${vo.idx}&title=${vo.title}">${vo.title}</a></c:if> <!-- 비밀글이 아니면 모두가 볼 수 있다. -->
		        <c:if test="${vo.pwd != '' && (sLevel == 0 || sMid == vo.mid)}">  <!-- 비밀글이면 '나'와 '관리자'만 볼 수 있다.(질문글은 당연히 본인 확인 가능) -->
		          <c:if test="${sLevel == 0 || sMid == vo.mid}">
		            <a href="adminQnaContent?pag=${pageVO.pag}&idx=${vo.idx}&title=${vo.title}">${vo.title}</a>
		          </c:if>
		        </c:if>
		        <c:if test="${vo.pwd != '' && (vo.nickName!=sName && sLevel != 0)}">
		          <c:if test="${vo.mid != sMid}">${vo.title}</c:if>
		        </c:if>
		      ...
		      <td>
		        ...
		        <c:if test="${vo.qnaAnswer == '답변대기'}">
		          <span class="badge badge-danger">
			          <c:if test="${sLevel == 0}"><a href="adminQnaContent?pag=${pageVO.pag}&idx=${vo.idx}&title=${vo.title}">${vo.qnaAnswer}</a></c:if>
