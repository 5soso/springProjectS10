...
create table qna (
  ...
  nickName  varchar(20)  not null,			/* qna 올린사람의 닉네임(성명) */
  title varchar(100) not null,					/* qna의 글 제목 */
  email varchar(50)  not null,					/* 이메일 */
  pwd 	varchar(20),										/* 비밀번호(필요없음... 여기선 비밀'질문/답변'글의 용도로 사용할것, 
  																			즉 체크박스에 비밀글로 체크하면 여기선 이곳에 '1234'를 저장했다.) */
  ...
  part varchar(10) not null,            /* 문의글 분류 [주문],[배송],[재고],[상품문의],[교환],[기타] */
...
select qna.*, cast(timestampdiff(minute, wDate, now())/60 as signed integer) as diffTime,
  (select qnaAnswer from qnaAdmin where qnaIdx=qna.idx ) as qnaSw
  from qna order by qnaIdx desc, qnaSw desc, ansLevel limit 0,10;
...
결과....
select * from qna order by qnaIdx desc, qnaSw desc, ansLevel;
idx		qnaIdx	qnaSw ansLevel
7			7				q			2
4			4				q			3
3			3				q			2
