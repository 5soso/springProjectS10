...
<head>
  <meta charset="UTF-8">
  <title>productMyOrder.jsp(회원 주문확인)</title>
  ...
  <script>
    ...
    function nWin(orderIdx) {
    	...
    	window.open(url,"productOrderBaesong","width=400px,height=450px");
    }
    ...
    function myOrderStatus() {
    	...
    	//location.href="productMyOrder?pag=${pageVO.pag}&startJumun="+startJumun+"&endJumun="+endJumun+"&conditionOrderStatus="+conditionOrderStatus;
    	location.href="productMyOrder?startJumun="+startJumun+"&endJumun="+endJumun+"&conditionOrderStatus="+conditionOrderStatus;
    }
    
    // 구매확정버튼 눌렀을 때
    function orderOk(orderIdx) {
    	let res = confirm("구매확정 하시겠습니까?");
    	if(!res) return false;
    	
    	$.ajax({
    		url : '${ctp}/product/productOrderOk',
    		type : 'post',
    		...
    		success : function(res){
    			...
    			else alert("구매 확정이 실패되었습니다. 관리자에게 문의하세요.");
    		},
    		error : function(){
    			alert("전송실패");
    		}
    ...
    function review(orderIdx) {
    	let url = "${ctp}/review/productReviewView?orderIdx="+orderIdx;
  ...
  <table class="table table-borderless">
    <tr>
      ...
      <td style="text-align:right;">
	      <a href="${ctp}/product/productCartList" class="btn btn-success btn-sm">장바구니조회</a>
	      <a href="${ctp}/product/productList" class="btn btn-primary btn-sm">계속쇼핑하기</a>
      </td>
    </tr>
  </table>
  <table class="table table-hover">
    ...
    <c:forEach var="vo" items="${vos}" varStatus="st">
      ...
      <tr>
        ...
        <c:set var="img" value="${fn:split(vo.thumbImg,'/')}" />
        <td style="text-align:center;"><br/><img src="${ctp}/product/${img[0]}" class="thumb" width="100px"/></td>
        <td align="left">
        ...
        <td style="text-align:center;"><br/>
          ...
          <c:if test="${vo.orderStatus eq '배송완료'}"><a href="javascript:orderOk('${vo.orderIdx}')" class="btn btn-info btn-sm mt-2">구매확정</a></c:if>
          <c:if test="${vo.orderStatus eq '구매확정'}"><a href="javascript:review('${vo.orderIdx}')" class="btn btn-warning btn-sm mt-2">리뷰작성</a></c:if>
        </td>
      </tr>
