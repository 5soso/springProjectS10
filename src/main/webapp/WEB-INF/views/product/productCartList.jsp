...
<html>
<head>
  <title>productCartList.jsp</title>
  ...
  <script>
    ...
    function onTotal(){
      ...
      // 배송비결정(60000원 이상이면 배송비는 0원으로 처리)
    ...
    function cartDelete(idx){
      ...
      $.ajax({
        type : "post",
        url  : "${ctp}/product/productCartDelete",
        data : {idx : idx},
        success:function() {
          location.reload();
        },
    ...
    function order(){
      ...
      let maxIdx = parseInt(document.getElementById("maxIdx").value);
      for(let i=minIdx;i<=maxIdx;i++){
        if($("#idx"+i).length != 0 && document.getElementById("idx"+i).checked){	// 해당 상품이 존재하면서, 구매한다고 체크가 되어있다면...
...
<div class="container" style="margin-top: 130px;">
  <h3 class="text-center">${sMid}님의 장바구니</h3>
	<br/>
		...
		<table class="table-bordered text-center" style="margin: auto; width:90%">
		  ...
		  <c:forEach var="listVO" items="${cartListVOS}">
		    <tr align="center">
		      ...
		      <td>
		        <c:set var="imgs" value="${fn:split(listVO.thumbImg, '/')}" />
		        <a href="${ctp}/product/productContentShop?idx=${listVO.productIdx}"><img src="${ctp}/product/${imgs[0]}" width="150px"/></a>
		      </td>
		      <td align="left">
		        <p class="contFont"><br/>
		          모델명 : <span style="color:orange;"><a href="${ctp}/product/productContentShop?idx=${listVO.productIdx}">${listVO.categorySubName} - ${listVO.productName}</a></span><br/>
  ...
  <p class="text-center">
	...
	<br/>
	<div class="text-center">
	  <button class="button button4" onClick="location.href='${ctp}/product/productList';">계속 쇼핑하기</button>
